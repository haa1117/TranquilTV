...
class PlaybackViewModel: ObservableObject {
    ...
    func togglePlayPause() {
        if isAudioOnlyMode {
            ...
            if isPlaying {
                player.pause()
                AnalyticsService.logPlaybackPaused(contentId: currentContent?.id ?? "", category: currentContent?.category ?? "")
            } else {
                player.play()
                AnalyticsService.logPlaybackResumed(contentId: currentContent?.id ?? "", category: currentContent?.category ?? "")
            }
        } else {
